	...
	initLayout: function() {
		...
		if (FR.apps && FR.apps.length > 0) {
			...
			Ext.each(FR.apps, function (item) {
				boxes.push({
					boxLabel: item.name + '<br><span class="silver" style="font-size:10px;">' + FR.T('Granted on %1 from %2').replace('%1', '<strong>'+item.date_created+'</strong>').replace('%2', '<strong>'+item.client_ip+'</strong>') + '<br>'+item.device_uuid+'</span>',
					name: 'revoke[]',
					inputValue: item.id
				});
			});
