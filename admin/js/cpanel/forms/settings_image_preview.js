...
FR.editSettings.formPanel = new FR.components.editForm({
				...
				},
				{
					...
					items: [
						{xtype: 'button', cls: 'fr-btn-default fr-btn-smaller fr-btn-nomargin', text: FR.T('Check path'), handler: function() {
							var par = {path: this.ownerCt.ownerCt.impath.getValue()};
	...
	FR.utils.getAjaxOutput(FR.URLRoot+'/?module=cpanel&section=settings&page=image_preview&action=checkImageMagick', par, output);
		...
		{
			xtype: 'fieldset',
			checkboxToggle: {tag: 'input', type: 'checkbox', name: this.checkboxName || this.id + '-checkbox', id: 'settings[thumbnails_libreoffice]'},
			checkboxName: 'settings[thumbnails_libreoffice]',
			title: FR.T('Enable LibreOffice support.'), animCollapse: true,
			...
			items: [
				...
				{
					xtype: 'textarea',
					fieldLabel: FR.T('Generate LibreOffice thumbnails for the following file types'),
		...
		{
			xtype: 'fieldset',
			checkboxToggle: {tag: 'input', type: 'checkbox', name: this.checkboxName || this.id + '-checkbox', id: 'settings[thumbnails_stl]'},
			checkboxName: 'settings[thumbnails_stl]',
			title: FR.T('Enable stl-thumb support.'), animCollapse: true,
	...
	tbar: [
		{
			...
			handler: function() {
				...
				extra['settings[thumbnails_libreoffice]'] = Ext.get('settings[thumbnails_libreoffice]').dom.checked ? 1:0;
				extra['settings[thumbnails_stl]'] = Ext.get('settings[thumbnails_stl]').dom.checked ? 1:0;
