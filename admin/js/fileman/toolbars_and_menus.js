FR.initToolbar = function() {
	...
	a.shareWithUsers = new FR.ContextAction({
		text: FR.T('With users'), iconCls: 'fa-user-plus', action: 'shareWithUsers', requires: [
			'single', 'non-empty', 'download', 'not-homefolder', 'share', 'isNotCollection'
		],
		isShareItem: true
	});
	...
	a.notifications = new FR.ContextAction({
		text: FR.T('Notifications'), iconCls: 'fa-bell', requires: ['single', 'folder', {sections: ['myfiles', 'sharedFolder', 'collection']}, 'isNotCollection',  function() {return Settings.allow_folder_notifications;}
		],
		menu: {
			listeners: {
				'beforeshow': function() {
	...
	a.more = new FR.ContextAction({
		...
		menu: [a.metadata, a.versioning, a.notifications, a.alog, a.indexMetadata, a.props]
	});
